...
- `make setup` can now bootstrap when PyYAML is missing — `pg-config.sh` and `docs-config.sh`
  (invoked at Make parse time) now probe for `yaml` and fall back to empty defaults on miss
