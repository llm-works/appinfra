...
### Added
- `RateLimiter(initial=True)` parameter — first call goes through immediately by default.
  Set `initial=False` for the previous behavior (wait one interval before first call).
  Matches `Ticker(initial=...)` semantics.
